...
export const ChannelAdmin: React.FC<Props> = ({ channels, reload }) => {
  ...
  return (
    <div className="space-y-6 pt-2">
      {/* Tab Navigation */}
      <div className="flex border-b border-gray-200 space-x-8 px-2 overflow-x-auto scrollbar-none">
         {[
           ...
           <button
             ...
             className={`pb-4 text-xs font-black uppercase tracking-widest flex items-center gap-2 transition-all relative ${
               activeTab === tab.id ? "text-brand-500" : "text-gray-500 hover:text-gray-900"
             }`}
           >
      ...
      <div className="min-h-[400px]">
          ...
          <div className="space-y-6">
            <div className="flex flex-col xl:flex-row gap-4 justify-between items-start xl:items-center">
              <div className="flex flex-col sm:flex-row gap-2 w-full xl:w-auto">
                <div className="relative flex-grow sm:w-72">
                  ...
                  <input
                    ...
                    className="w-full bg-gray-100/50 border border-gray-300 rounded-xl py-2 pl-9 pr-4 text-xs text-gray-900 placeholder:text-neutral-600 focus:outline-none focus:border-brand-500/50"
                  />
                </div>
                
                {/* Dynamically populate categories filter */}
                <div className="flex items-center gap-2 bg-gray-100/50 border border-gray-300 rounded-xl px-3 sm:w-56 shrink-0">
                  <Filter size={12} className="text-gray-500 shrink-0" />
                  <select
                    value={activesCategory}
                    onChange={e => setActivesCategory(e.target.value)}
                    className="bg-transparent text-xs text-gray-900 font-bold uppercase tracking-tight focus:outline-none w-full py-2 cursor-pointer"
                  >
              ...
              <div className="flex flex-wrap gap-2 w-full sm:w-auto">
                <button
                  ...
                  className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2 bg-gray-100 hover:bg-gray-200 border border-gray-300 hover:border-brand-500/50 rounded-xl text-[10px] font-black uppercase tracking-wider text-brand-400 transition-all disabled:opacity-50"
                >
                  {testingAll ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
              {filteredChannels.map(c => {
                ...
                return (
                  <div key={c.id} className="p-4 bg-gray-100 border border-gray-200 rounded-2xl md:rounded-[1.8rem] group active:scale-[0.99] transition-all hover:bg-gray-200/50 hover:border-gray-300 hover:shadow-xl">
                    {isEditing ? (
                      <div className="space-y-3">
                         <div className="space-y-1">
                            <label className="text-[8px] font-black text-gray-500 uppercase tracking-widest px-1">Nom du canal</label>
                            <input 
                              type="text" 
                              value={editForm.name} 
                              onChange={e => setEditForm({...editForm, name: e.target.value})} 
                              className="w-full bg-white/60 border border-gray-300 rounded-xl px-3 py-2 text-xs text-gray-900 focus:border-brand-500 outline-none" 
                              placeholder="Nom"
                            />
                         </div>
                         <div className="space-y-1">
                            <label className="text-[8px] font-black text-gray-500 uppercase tracking-widest px-1">Catégorie</label>
                            <input 
                              type="text" 
                         ...
                         <div className="flex gap-2 justify-end pt-1">
                            <button onClick={() => setEditingId(null)} className="px-4 py-2 text-[9px] font-black text-gray-500 hover:text-gray-900 uppercase tracking-widest font-mono">Annuler</button>
                            <button onClick={() => saveEdit(c.id)} className="px-4 py-2 bg-brand-500 text-gray-900 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-lg shadow-brand-500/20">Sauvegarder</button>
                         </div>
                      </div>
                    ) : (
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center p-2 overflow-hidden border border-gray-200 group-hover:border-gray-300 transition-colors shadow-inner">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                               <h4 className="text-sm font-black text-gray-900 line-clamp-1 tracking-tight">{c.name}</h4>
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                               <p className="text-[9px] text-gray-500 font-black uppercase tracking-[0.15em]">{c.categoryOverride || c.category}</p>
                               ...
                               {streamStates[String(c.id)] ? (
                                 ...
                                 <span className="text-[8px] text-gray-500 font-mono font-black uppercase tracking-widest">Inconnu</span>
                               )}
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all translate-x-2 group-hover:translate-x-0">
                          <button 
                            ...
                            className="p-2.5 text-gray-500 hover:text-brand-500 hover:bg-brand-500/10 rounded-xl transition-all disabled:opacity-50" 
                            title="Tester le flux"
                          >
                             <Activity size={16} />
                          </button>
                          <button onClick={() => handleEdit(c)} className="p-2.5 text-gray-500 hover:text-gray-900 hover:bg-black/5 rounded-xl transition-all" title="Modifier"><Edit size={16} /></button>
        ...
        {activeTab === 'discovery' && (
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row gap-4">
               <div className="relative flex-grow">
                 ...
                 <input
                   ...
                   className="w-full bg-gray-100/50 border border-gray-300 rounded-xl py-2 pl-9 pr-4 text-xs text-gray-900 focus:outline-none focus:border-brand-500/50"
                 />
               </div>
               <div className="flex items-center gap-2 bg-gray-100/50 border border-gray-300 rounded-xl px-3 min-w-[140px]">
                  <Filter size={12} className="text-gray-500" />
                  <select 
                    value={selectedCountry} 
                    onChange={e => setSelectedCountry(e.target.value)}
                    className="bg-transparent text-xs text-gray-900 font-bold uppercase tracking-tight focus:outline-none w-full py-2"
                  >
                    {catalogCountries.map(country => (
                      <option key={country} value={country} className="bg-gray-100">{country}</option>
                    ))}
                  </select>
               </div>
            </div>
            ...
            {isCatalogLoading ? (
              <div className="flex items-center justify-center p-20 animate-pulse text-xs font-black uppercase text-gray-500 tracking-widest">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                {catalog.map(item => {
                  ...
                  return (
                    <div key={item.id} className="p-3 bg-gray-100 border border-gray-200 rounded-xl flex items-center justify-between group hover:border-gray-300 transition-all">
                      <div className="flex items-center gap-3 overflow-hidden">
                        <div className="w-8 h-8 bg-white rounded p-1 shrink-0 flex items-center justify-center">
                          <ChannelLogo logo={item.logo} name={item.name} className="max-w-full max-h-full object-contain" />
                        </div>
                        <span className="text-[11px] font-bold text-gray-900 truncate pr-2">{item.name}</span>
                      </div>
                      <button
                        ...
                        className={`p-1.5 px-2.5 rounded-lg text-[8px] font-black uppercase tracking-widest transition-all ${
                          isActive 
                          ? "bg-red-500/10 text-red-500 border border-red-500/20 hover:bg-red-500 hover:text-gray-900" 
                          : "bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 hover:bg-emerald-500 hover:text-gray-900"
                        }`}
                      >
        ...
        {activeTab === 'add' && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto py-4">
            <form onSubmit={handleAddChannel} className="p-8 bg-gray-100 border border-gray-200 rounded-[2rem] space-y-6 shadow-2xl">
              <div className="flex items-center gap-4">
                 <div className="w-12 h-12 bg-brand-500/10 rounded-2xl flex items-center justify-center text-brand-500">
                    <Plus size={24} />
                 </div>
                 <div>
                    <h4 className="text-base font-black text-gray-900 uppercase tracking-tighter">Ajouter un flux M3U8</h4>
                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Intégrez vos propres chaînes personnalisées</p>
                 </div>
              </div>
              ...
              <div className="space-y-4 pt-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                   <div className="space-y-1.5">
                      <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest ml-1">Nom de la chaîne</label>
                      <input
                        type="text"
                        placeholder="Ex: My Movie Channel"
                        value={addForm.name}
                        onChange={e => setAddForm({ ...addForm, name: e.target.value })}
                        className="w-full bg-gray-50 border border-gray-300 rounded-xl px-4 py-3 text-sm text-gray-900 focus:outline-none focus:border-brand-500 transition-all font-medium"
                      />
                   </div>
                   <div className="space-y-1.5">
                      <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest ml-1">Catégorie</label>
                      <select
                        value={addForm.category}
                        onChange={e => setAddForm({ ...addForm, category: e.target.value })}
                        className="w-full bg-gray-50 border border-gray-300 rounded-xl px-4 py-3 text-sm text-gray-900 focus:outline-none focus:border-brand-500 font-bold uppercase tracking-tight"
                      >
                ...
                <div className="space-y-1.5">
                  <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest ml-1">URL du flux (.m3u8 / .ts)</label>
                  <input
                    type="text"
                    placeholder="https://..."
                    value={addForm.streamUrl}
                    onChange={e => setAddForm({ ...addForm, streamUrl: e.target.value })}
                    className="w-full bg-gray-50 border border-gray-300 rounded-xl px-4 py-3 text-xs text-gray-900 font-mono focus:outline-none focus:border-brand-500"
                  />
                </div>

                <div className="space-y-1.5">
                   <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest ml-1">URL de l'image (Logo)</label>
                   <input
                    type="text"
                    placeholder="https://..."
                    value={addForm.logo}
                    onChange={e => setAddForm({ ...addForm, logo: e.target.value })}
                    className="w-full bg-gray-50 border border-gray-300 rounded-xl px-4 py-3 text-xs text-gray-900 focus:outline-none focus:border-brand-500"
                   />
                </div>
              </div>
              ...
              <button
                ...
                className="w-full py-4 bg-brand-500 hover:bg-brand-600 rounded-2xl flex items-center justify-center gap-3 active:scale-[0.98] transition-all text-gray-900 font-black uppercase tracking-[0.2em] shadow-xl shadow-brand-500/20"
              >
        ...
        {activeTab === 'lcn' && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            <div className="p-6 bg-gray-100 border border-gray-200 rounded-2xl md:rounded-[2rem] space-y-6">
               <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div className="flex items-center gap-4">
                     ...
                     <div>
                        <h4 className="text-base font-black text-gray-900 uppercase tracking-tighter">Numérotation LCN</h4>
                        <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Optimisez la disposition automatique des bouquets TV français</p>
                     </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 w-full md:w-auto">
                     <button
                       ...
                       className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2.5 bg-gray-50 border border-gray-300 hover:border-brand-500/50 hover:bg-gray-200 rounded-xl text-[10px] font-black uppercase tracking-wider text-brand-400 transition-all cursor-pointer disabled:opacity-50"
                     >
                        {isLoadingLcn ? (
                     ...
                     <button
                       ...
                       disabled={isLoadingLcn}
                       className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2.5 bg-gray-50 border border-gray-300 hover:border-red-500/50 hover:bg-red-500/10 rounded-xl text-[10px] font-black uppercase tracking-wider text-red-400 transition-all cursor-pointer disabled:opacity-50"
                     >
                        <RotateCcw size={12} />
                        Réinitialiser
                     </button>
                     <button
                       ...
                       disabled={isLoadingLcn}
                       className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-2.5 bg-brand-500 hover:bg-brand-600 rounded-xl text-[10px] font-black uppercase tracking-wider text-gray-900 transition-all shadow-lg shadow-brand-500/10 cursor-pointer disabled:opacity-50"
                     >
                        {isLoadingLcn ? (
                          <Loader2 size={12} className="animate-spin" />
                        ) : (
               ...
               <div className="relative w-full">
                 ...
                 <input
                   ...
                   className="w-full bg-white/45 border border-gray-300 rounded-xl py-2 pl-9 pr-4 text-xs text-gray-900 placeholder:text-neutral-600 focus:outline-none focus:border-brand-500/50"
                 />
               </div>

               <div className="max-h-[50vh] overflow-y-auto pr-2 custom-scrollbar space-y-2">
                  {isLoadingLcn && Object.keys(lcnMap).length === 0 ? (
                     <div className="flex flex-col items-center justify-center py-20 text-xs font-black uppercase text-gray-500 tracking-widest gap-2">
                     ...
                     Object.entries(lcnMap)
                       ...
                       .map(([core, currentVal]) => (
                          <div key={core} className="flex items-center justify-between p-3.5 bg-gray-50/40 border border-gray-200 rounded-xl hover:border-gray-300 transition-all">
                             <div className="flex items-center gap-3">
                                <div className="w-7 h-7 bg-black/5 rounded-lg flex items-center justify-center font-bold text-xs text-gray-900 uppercase font-mono">
                                   #
                                </div>
                                <div className="space-y-0.5">
                                   <p className="text-xs font-black text-gray-900 font-mono tracking-tight">{core}</p>
                                   <div className="flex items-center gap-2">
                             ...
                             <div className="flex items-center gap-3">
                                <input
                                  ...
                                  className="w-20 bg-gray-100 border border-gray-300 focus:border-brand-500/50 rounded-lg px-3 py-1.5 text-xs text-center text-gray-900 font-black animate-none"
                                />
                             </div>
                          </div>
                       ))
        ...
        {activeTab === 'system' && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {/* Stats Section */}
              <div className="lg:col-span-1 bg-gray-100 border border-gray-200 p-6 rounded-2xl md:rounded-[2rem] space-y-4">
                <div className="flex items-center gap-3">
                  <div className="p-3 bg-brand-500/10 text-brand-400 rounded-xl">
                    <Activity size={20} />
                  </div>
                  <div>
                    <h4 className="text-sm font-black text-gray-900 uppercase tracking-tight">Rapport d'État</h4>
                    <p className="text-[9px] text-gray-500 font-bold uppercase tracking-wider">État actuel du tuner local</p>
                  </div>
                </div>

                <div className="space-y-3 pt-2">
                  <div className="flex justify-between items-center p-3 bg-white/40 rounded-xl border border-white/[0.02]">
                    <span className="text-[10px] text-gray-600 font-bold uppercase tracking-wider">Total Chaînes</span>
                    <span className="text-sm font-black font-mono text-gray-900">{channels.length}</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-white/40 rounded-xl border border-white/[0.02]">
                    <span className="text-[10px] text-gray-600 font-bold uppercase tracking-wider">Chaînes Personnalisées</span>
                    <span className="text-sm font-black font-mono text-amber-400">
                      {channels.filter(c => String(c.id).includes('custom_')).length}
                    </span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-white/40 rounded-xl border border-white/[0.02]">
                    <span className="text-[10px] text-gray-600 font-bold uppercase tracking-wider">LCN Mappés</span>
              ...
              <div className="lg:col-span-2 bg-gray-100 border border-gray-200 p-6 rounded-2xl md:rounded-[2rem] space-y-6">
                <div className="flex items-center gap-3">
                  <div className="p-3 bg-gray-200 text-gray-600 rounded-xl">
                    <Database size={20} />
                  </div>
                  <div>
                    <h4 className="text-sm font-black text-gray-900 uppercase tracking-tight">Gestion des Données (Sauvegardes)</h4>
                    <p className="text-[9px] text-gray-500 font-bold uppercase tracking-wider">Sauvegardez vos numérotations, logos et flux personnalisés</p>
                  </div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                  {/* Backup Card */}
                  <div className="p-4 bg-white/30 rounded-xl border border-gray-200 hover:border-brand-500/20 transition-all space-y-3 flex flex-col justify-between">
                    <div>
                    ...
                    <a
                      ...
                      className="w-full py-2 bg-brand-500/10 hover:bg-brand-500 text-brand-400 hover:text-gray-900 rounded-xl text-[9px] font-black uppercase tracking-wider text-center transition-all"
                    >
                      Télécharger .json
                    </a>
                  </div>
                  ...
                  <div className="p-4 bg-white/30 rounded-xl border border-gray-200 hover:border-brand-500/20 transition-all space-y-3 flex flex-col justify-between">
                    ...
                    <label className="w-full py-2 bg-emerald-500/10 hover:bg-emerald-500 border border-emerald-500/20 text-emerald-400 hover:text-gray-900 rounded-xl text-[9px] font-black uppercase tracking-wider text-center transition-all cursor-pointer block">
