...
export function EpgTimeline({ channelName, onClose, onProgramClick }: EpgTimelineProps) {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 20 }}
      className="flex flex-col h-full bg-gray-50 border border-gray-200 rounded-3xl overflow-hidden shadow-2xl"
    >
      {/* Header */}
      <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gray-100/50 backdrop-blur-xl">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-xl font-black text-gray-900 uppercase tracking-tighter">Guide TV 24h</h3>
            <p className="text-[10px] font-black text-[#FF7900] uppercase tracking-widest">{channelName}</p>
          </div>
        </div>
        <button 
          onClick={onClose}
          className="p-3 bg-black/5 hover:bg-black/10 text-gray-600 hover:text-gray-900 rounded-2xl transition-all active:scale-95 border border-gray-200"
        >
          <X size={20} />
        </button>
      </div>
      ...
      <div className="flex-grow overflow-y-auto p-4 sm:p-6 space-y-4">
        {loading ? (
          <div className="flex flex-col items-center justify-center h-64 gap-4">
            ...
            <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Chargement de la grille...</p>
          </div>
        ) : error ? (
          <div className="flex flex-col items-center justify-center h-64 gap-4 text-center px-8">
            ...
            <button 
              ...
              className="px-6 py-2.5 bg-gray-100 border border-gray-300 rounded-xl text-[10px] font-black text-gray-900 uppercase tracking-widest hover:bg-gray-200 transition-all"
            >
              Réessayer
            </button>
          </div>
          ...
          <div className="flex flex-col items-center justify-center h-64 gap-4 text-center">
            ...
            <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Aucun programme trouvé</p>
          </div>
        ) : (
          <div className="relative">
            {/* Timeline Vertical Line */}
            <div className="absolute left-[54px] top-4 bottom-4 w-px bg-black/5" />
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-shrink-0 w-14 text-right pt-1">
                      <span className={`text-xs font-mono font-black ${current ? 'text-brand-500' : 'text-gray-500'}`}>
                    ...
                    <div className="relative mt-2 flex-shrink-0">
                      <div className={`w-3 h-3 rounded-full border-2 bg-gray-50 z-10 relative transition-transform duration-300 group-hover:scale-125 ${
                        current 
                    ...
                    <div 
                      ...
                      className={`flex-grow p-5 rounded-[1.5rem] border transition-all duration-300 relative overflow-hidden cursor-help ${
                        current 
                          ...
                          : 'bg-gray-100/50 border-gray-200 hover:border-gray-300 hover:bg-gray-200/50'
                      }`}
                    >
                      {current && (
                         <div className="absolute top-0 right-0 p-3">
                            <span className="flex items-center gap-1.5 px-2 py-0.5 bg-red-600 text-[8px] font-black text-gray-900 rounded-md uppercase tracking-widest animate-pulse shadow-lg">
                      ...
                      <div className="flex flex-col md:flex-row gap-5">
                        {prog.image && (
                          <div className="w-full md:w-32 aspect-video md:aspect-square rounded-2xl overflow-hidden bg-white shrink-0 border border-gray-200">
                            <img 
                              src={prog.image} 
                              alt="" 
                              loading="lazy"
                        ...
                        <div className="flex-grow">
                          <h4 className={`text-sm md:text-base font-black tracking-tight mb-2 ${current ? 'text-gray-900' : 'text-gray-700'}`}>
                            {prog.title}
                          </h4>
                          <div className="flex items-center gap-3 mb-3">
                            <span className="text-[9px] font-mono font-bold text-gray-500 bg-gray-50 px-2 py-0.5 rounded-full border border-gray-200">
