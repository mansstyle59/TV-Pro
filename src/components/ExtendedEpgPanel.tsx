...
export function ExtendedEpgPanel({ channel, onClose, onPlay }: ExtendedEpgPanelProps) {
  ...
  return (
    <motion.div 
      ...
      className="absolute inset-x-0 top-0 bottom-0 bg-neutral-950 flex flex-col h-full z-50 overflow-hidden"
    >
      {/* Upper Navigation Row */}
      <div className="p-4 bg-neutral-900/60 border-b border-white/[0.04] backdrop-blur-md flex items-center justify-between shrink-0">
        <button
          onClick={onClose}
          className="flex items-center gap-1 text-xs font-black text-neutral-400 hover:text-white uppercase tracking-wider transition-colors py-1 cursor-pointer"
        >
          <ChevronLeft size={16} className="text-[#FF7900]" />
          <span>Retour</span>
        </button>

        <div className="flex items-center gap-1.5 bg-black/40 px-2 py-1 rounded-lg border border-white/5 font-mono text-[9px] text-[#FF7900] font-bold">
      ...
      <div className="flex-grow flex flex-col overflow-y-auto scrollbar-thin scrollbar-thumb-neutral-800">
        ...
        <div className="p-4 bg-gradient-to-b from-neutral-900/40 via-neutral-950/20 to-neutral-950 border-b border-white/[0.03] space-y-3 shrink-0">
          <div className="flex items-start gap-3.5">
            ...
            <div className="min-w-0 flex-grow select-text">
              ...
              <p className="text-[9px] text-neutral-400 font-medium tracking-wide mt-1 capitalize">
                Catégorie : <span className="font-semibold text-neutral-300">{channel.category || "Inconnue"}</span>
              </p>
            </div>
        ...
        {detailedProgram ? (
          <div className="px-4 pb-2 shrink-0">
            <div className="bg-neutral-900/50 rounded-2xl border border-white/[0.04] p-4 space-y-3 relative overflow-hidden bg-radial-at-t from-neutral-900/20 to-neutral-950">
              
              {/* Image banner backdrop if present */}
              {detailedProgram.image ? (
                <div className="w-full h-24 rounded-lg overflow-hidden relative border border-white/5 select-none bg-black">
                  <img 
                  ...
                  {isCurrentActive(detailedProgram) && (
                    <span className="absolute top-2 right-2 flex items-center gap-1 px-1.5 py-0.5 bg-red-600 text-[7px] font-black tracking-widest text-white rounded uppercase animate-pulse shadow-sm">
                ...
                isCurrentActive(detailedProgram) && (
                  <div className="flex items-center justify-between">
                    <span className="flex items-center gap-1 px-1.5 py-0.5 bg-red-600 text-[7px] font-black tracking-widest text-white rounded uppercase animate-pulse">
                      <Play size={7} fill="currentColor" />
                      Direct
                    </span>
                  </div>
              ...
              <div className="space-y-1 select-text">
                ...
                <div className="flex items-center gap-2 text-[9px] text-neutral-400">
                  ...
                  {detailedProgram.category && (
                    <span className="text-neutral-500 font-extrabold uppercase tracking-widest text-[8px]">
                      {detailedProgram.category}
                    </span>
                  )}
                </div>
              ...
              {detailedProgram.desc ? (
                <div className="bg-black/20 p-2.5 rounded-xl border border-white/[0.02] max-h-36 overflow-y-auto select-text">
                  <p className="text-[10px] text-neutral-400 font-medium leading-relaxed">
                    {detailedProgram.desc}
                  </p>
                </div>
              ) : (
                <p className="text-[10px] text-neutral-500 italic font-medium">
              ...
              {showBackToLive && (
                <button
                  ...
                  className="w-full mt-1.5 py-1.5 bg-neutral-950/90 border border-[#FF7900]/25 hover:border-[#FF7900]/50 text-neutral-300 hover:text-white rounded-xl text-[8px] font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 cursor-pointer hover:shadow-xs hover:scale-[1.01]"
                >
        ...
        <div className="p-4 border-t border-white/[0.03] space-y-3 flex-grow flex flex-col">
          <div className="flex items-center justify-between">
            <h5 className="text-[9px] font-black uppercase text-neutral-500 tracking-widest flex items-center gap-1.5">
            ...
            <button
              ...
              className={`p-1 text-neutral-500 hover:text-white transition-colors cursor-pointer ${loading ? "animate-spin" : ""}`}
          ...
          {loading ? (
            <div className="flex-grow flex flex-col items-center justify-center py-10 gap-2.5">
              ...
              <p className="text-[8px] font-mono font-bold text-neutral-500 tracking-widest uppercase">
            ...
            <div className="flex-grow flex flex-col items-center justify-center py-10 text-center gap-2">
              ...
              <p className="text-[9px] text-neutral-400 font-medium">Chargement EPG restreint</p>
            </div>
          ) : programmes.length === 0 ? (
            <div className="flex-grow flex flex-col items-center justify-center py-10 text-center">
              <p className="text-[9px] text-neutral-500">Aucun log programme enregistré.</p>
            </div>
          ) : (
            <div className="relative pl-3 space-y-2">
              {/* Vertical dotted timeline track anchor */}
              <div className="absolute left-1 top-2 bottom-2 w-[1px] bg-neutral-900 border-l border-dashed border-white/10" />
              ...
              {programmes.map((prog, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-2.5 rounded-xl border flex items-start gap-3 transition-all relative cursor-pointer outline-none ${
                      isSelected 
                        ...
                        : live 
                        ? "bg-[#FF7900]/5 border-white/5 hover:border-white/10" 
                  ...
                  >
                    ...
                    <div className="absolute -left-[14px] top-4.5 z-10 w-2 h-2 rounded-full bg-neutral-950 border border-white/10 flex items-center justify-center">
                    ...
                    <span className={`text-[8.5px] font-mono font-black shrink-0 pt-0.5 min-w-8 ${
                      live ? "text-[#FF7900]" : "text-neutral-500"
                    }`}>
                    ...
                    <div className="flex-grow min-w-0 pr-1.5">
                      <h6 className={`text-[10.5px] font-semibold leading-tight truncate ${
                        isSelected || live ? "text-white" : "text-neutral-300"
                      }`}>
                        {prog.title}
                      </h6>
                    ...
                    <div className="shrink-0 pt-1 text-neutral-600 hover:text-neutral-400">
